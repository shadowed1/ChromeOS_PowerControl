...
detect_gpu_freq() {
    ...
    if [[ -d /sys/class/drm ]]; then
        if grep -qi "mediatek" /sys/class/drm/*/device/uevent 2>/dev/null; then
            SOC_LABEL="mali"
            echo "${YELLOW}[*] Detected mali GPU${RESET}"
...
declare -a files=(
  ...
  "sleepcontrol" "deep_suspend.sh" "deep_suspend_battery.sh"
...
declare -A categories=(
  ["PowerControl"]="MAX_TEMP MIN_TEMP MAX_PERF_PCT MIN_PERF_PCT HOTZONE CPU_POLL RAMP_UP RAMP_DOWN POWERCONTROL_STATUS"
  ["BatteryControl"]="CHARGE_MAX BATTERYCONTROL_STATUS"
  ["FanControl"]="MIN_FAN MAX_FAN FAN_MIN_TEMP FAN_MAX_TEMP STEP_UP STEP_DOWN FAN_POLL FANCONTROL_STATUS"
  ["GPUControl"]="GPU_MAX_FREQ GPUCONTROL_STATUS"
  ["SleepControl"]="BATTERY_DELAY BATTERY_BACKLIGHT BATTERY_DIM_DELAY POWER_DELAY POWER_BACKLIGHT POWER_DIM_DELAY AUDIO_DETECTION_BATTERY AUDIO_DETECTION_POWER LIDSLEEP_BATTERY LIDSLEEP_POWER SLEEPCONTROL_STATUS"
...
if [[ -z "${POWERCONTROL_STATUS}" ]]; then POWERCONTROL_STATUS=0; fi
if [[ -z "${CHARGE_MAX}" ]]; then CHARGE_MAX=77; fi
if [[ -z "${BATTERYCONTROL_STATUS}" ]]; then BATTERYCONTROL_STATUS=0; fi
...
if [[ -z "${FANCONTROL_STATUS}" ]]; then FANCONTROL_STATUS=0; fi
if [[ -z "${GPUCONTROL_STATUS}" ]]; then GPUCONTROL_STATUS=0; fi
...
declare -A defaults=(
  ...
  [RAMP_UP]=$RAMP_UP
  [RAMP_DOWN]=$RAMP_DOWN
  [POWERCONTROL_STATUS]=$POWERCONTROL_STATUS
  [CHARGE_MAX]=$CHARGE_MAX
  [BATTERYCONTROL_STATUS]=$BATTERYCONTROL_STATUS
  [MIN_FAN]=$MIN_FAN
  [MAX_FAN]=$MAX_FAN
  ...
  [STEP_DOWN]=$STEP_DOWN
  [FAN_POLL]=$FAN_POLL
  [FANCONTROLCONTROL_STATUS]=$FANCONTROL_STATUS
  [GPU_MAX_FREQ]=$GPU_MAX_FREQ
  [GPU_TYPE]=$GPU_TYPE
  [GPUCONTROLCONTROL_STATUS]=$GPUCONTROL_STATUS
  ...
  [SLEEPCONTROLCONTROL_STATUS]=$SLEEPCONTROL_STATUS
  [PERF_PATH]=$PERF_PATH

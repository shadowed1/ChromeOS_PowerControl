...
- 0.57: `Fixed audio detection settings and duplicate log entries for SleepControl. Installer will now stop scripts prior to downloading to fix bash error. Added icon support for GUI app running in Crostini. Fixed version command so Y is default. Thanks to Denny for reporting.` <br><br>
